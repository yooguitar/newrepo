	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		/*
		 방법 1.
			 ...
			 시큐리티 필터 체인을 만드는 방법임. 시큐리티는 사용하되 formLogin()은 사용하지 않겠다!
			 하지만 메소드 체이닝하는 방법은 7.0에서 사라질 문법이므로 사용 안할 것
		 ...
		 방법 2.
			return httpSecurity.formLogin(new Customizer<FormLoginConfigurer<HttpSecurity>>() {
				...
				대안으로 익명클래스를 구현하는 방법을 사용했다.
				하지만 이 방법은 제외할 클래스가 많을 경우 중복 코드가 많고, 
				코드가 복잡하기 때문에 부모 클래스(AbstractHttpConfigurere)를 상속받아 다형성을 적용하는 방법을 사용할 것
		*/
